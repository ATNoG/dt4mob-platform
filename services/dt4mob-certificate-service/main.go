...
func main() {
	...
	filtered := slices.DeleteFunc(config.PermittedRoles, func(s string) bool {
		return strings.TrimSpace(s) == ""
	})
	config.PermittedRoles = filtered
	if len(config.PermittedRoles) == 0 {
	...
	api.UseMiddleware(verifier.NewRoleMiddleware(api, config.PermittedRoles))
